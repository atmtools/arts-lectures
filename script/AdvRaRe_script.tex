...
\author{Stefan A Buehler, Lukas Kluft, Christoph Sauter, Jakob Doerr, \colorbox{red}{FIXME} all that worked on
the script}
\date{\today}
...
Every free molecule has 3 \textbf{Moments of inertia} $I_A$, $I_B$, $I_C$ (\textit{Trägheitsmomente}), since there are 3 principal axes of rotation in the x-y-z-plane (Fig. \ref{Fig_moments_inertia}). We can use the Moments of inertia to classify the molecules and devide them into groups:
...
\end{equation}
\colorbox{red}{FIXME: What are the names of $k$ and $\mu$?}\\
