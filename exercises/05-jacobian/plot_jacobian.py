...
plt.figure(1)#'PaperPosition', [.25 .25 8 6]);
if freq_ind > 0:
  plt.subplot(221)
else:
  plt.subplot(211)
...
if freq_ind > 0:
    ...
    plt.text(4, 60, str(alt[np.argmin(jac[freq_ind, :]),0,0].round(1)/1000)+'km')
...
pathname = './plots/';
if freq_ind < 0:
    figname = 'bt_op_part1.png'
    plt.tight_layout()
    plt.savefig(pathname+figname, dpi=300)
else:
    figname = 'jac_'+str(np.round((freq[freq_ind] / 10**9))) +'GHz.png'
    plt.tight_layout()
    plt.savefig(pathname+figname, dpi=300)
