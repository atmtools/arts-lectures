...
plot(wave_num, bb_inten_Ts);
plot(wave_num, bb_inten_275);
plot(wave_num, bb_inten_250);
plot(wave_num, bb_inten_225);
...
text(1500, 3e-12, strcat('OLR = ',  num2str(int_spectra, '%5.2f'), ...
     ' Wm^{-2}Sr^{-1}'));

%% Save figure
pathname = './plots';
figname = ['olr' scene '.png'];
print(gcf, fullfile(pathname, figname), '-dpng', '-r300');
