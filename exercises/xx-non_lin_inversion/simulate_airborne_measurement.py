...
# atmospheric data
atms=xml.load('input/atmospheres_true.xml')
auxs=xml.load('input/aux1d_true.xml')
...
for band, suffix in zip(bands, suffixes):
    ...
    for i in range(len(atms)):
        ...
        atm=atms[i]
        y[i,:] = nlo.forward_model(ws, atm, surface_reflectivity, surface_temperature, sensor_pos, sensor_los)
    ...
    Y.savexml(f'input/y_true_{suffix}.xml')
    Y_obs.savexml(f'input/y_obs_{suffix}.xml')
    F_grid.savexml(f'input/f_grid_{suffix}.xml')

    print('ddd')


Lat=nlo.pa.arts.Vector(lat)
Lat.savexml('input/lat.xml')
...
for i, band in enumerate(bands):
    ...
    ax[i].set_prop_cycle(color=cmap)
    for j,f in enumerate(results[(f'f_grid_{band}')]):
        ax[i].plot(lat, results[(f'y_obs_{band}')][:,j],'--', label=f'{f/1e9:0.2f} GHz (obs)')
